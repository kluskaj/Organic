...
class OI_VIS:
    ...
    def __init__(self, timeobs, int_time, visamp, visamperr, visphi, visphierr, flag, ucoord,
                 vcoord, wavelength, target, array=None, station=(None,None), cflux=None,
                 cfluxerr=None, viscf=None, viscferr=None, visphicf=None, visphicferr=None):
        ...
        if viscf != None: self._viscf = np.array(viscf, dtype=double).reshape(-1)
        else: self._viscf = None
        if viscferr != None: self._viscferr = np.array(viscferr, dtype=double).reshape(-1)
        else: self._viscferr = None
        if visphicf != None: self._visphicf = np.array(visphicf, dtype=double).reshape(-1)
        else: self._visphicf = None
        if visphicferr != None: self._visphicferr = np.array(visphicferr, dtype=double).reshape(-1)
        else: self._visphicferr = None
    ...
    def __getattr__(self, attrname):
        ...
        elif attrname in ('cflux', 'cfluxerr','viscf','viscferr','visphicf','visphicferr'):
    ...
    def __setattr__(self, attrname, value):
        if attrname in ('visamp', 'visamperr', 'visphi', 'visphierr', 'cflux', 'cfluxerr','viscf','viscferr','visphicf','visphicferr'):
...
class oifits:
    ...
    def __init__(self):
        ...
        self._allvisb = np.zeros(1,dtype=[('viscf','f8'),('viscferr','f8'),('visphicf','f8'),('visphicferr','f8')]).view(ma.MaskedArray)
    ...
    def __getattr__(self,attrname):
        ...
        elif attrname == 'allvisb':
            self._allvisb = self._allvisb[:1]
            temp = self._allvisb.copy()
            allmask = [0]
            for i in range(len(self.vis)):
                # extract the actual observables
                viscfdata = getattr(self.vis[i],'viscf')
                viscferr = getattr(self.vis[i],'viscferr')
                visphicfdata = getattr(self.vis[i],'visphicf')
                visphicferr = getattr(self.vis[i],'visphicferr')

                # return only the valid datapoints
                if viscferr != None:
                    mask = viscferr.mask
                elif visphicferr != None:
                    mask = visphicferr.mask
                elif viscfdata != None:
                    mask = viscfdata.mask
                elif visphicfdata != None:
                    mask = visphicferr.mask
                else:
                    print( 'No CF columns present in this file, hence no allvisb!')
                    raise (AttributeError(attrname))

                # fill the allviscf array
                for j in range(len(mask)):
                    temp[0] = (None,None,None,None)
                    if viscfdata != None:
                        temp[0][0] = viscfdata.data[j]
                    if viscferr != None:
                        temp[0][1] = viscferr.data[j]
                    if visphicfdata != None:
                        temp[0][2] = visphicfdata.data[j]
                    if visphicferr != None:
                        temp[0][3] = visphicferr.data[j]
                    allmask.append(mask[j])
                    self._allvisb = np.append(self._allvisb,temp)

            self._allvisb.mask = allmask
            return copy.deepcopy(self._allvisb[1:])

        else:
            raise AttributeError( attrname)

    def setAllAttr(self,attrname,value,set_err=False):
        """
        ...
        if attrname == 'allvis':
            if (
                ...
                for i in range(len(self.vis)):
                    ...
                    if set_err == True:
                        setattr(self.vis[i],'visamperr',np.array(value.data['visamperr'][index0:indexn],dtype=double).reshape(-1))
                        setattr(self.vis[i],'visphierr',np.array(value.data['visphierr'][index0:indexn],dtype=double).reshape(-1))
                    index0 = indexn
        ...
        if attrname == 'allvis2':
            if (
                ...
                for i in range(len(self.vis2)):
                    ...
                    if set_err == True:
                        setattr(self.vis2[i],'vis2err',np.array(value['vis2err'][index0:indexn],dtype=double).reshape(-1))
                    index0 = indexn
        ...
        if attrname == 'allt3':
            if (
                ...
                for i in range(len(self.t3)):
                    ...
                    if set_err == True:
                        setattr(self.t3[i],'t3amperr',np.array(value.data['t3amperr'][index0:indexn],dtype=double).reshape(-1))
                        setattr(self.t3[i],'t3phierr',np.array(value.data['t3phierr'][index0:indexn],dtype=double).reshape(-1))
                    index0 = indexn
        ...
        if self.vis.size:
            ...
            for vis in self.vis:
                ...
                if key in tables.keys():
                    ...
                    data = tables[key] = {'target_id':[], 'time':[], 'mjd':[], 'int_time':[],
                                          ...
                                          'cflux':[], 'cfluxerr':[], 'viscf':[], 'viscferr':[],
                                          'visphicf':[], 'visphicferr':[], 'ucoord':[], 'vcoord':[],
                                          'sta_index':[], 'flag':[]}
                ...
                if nwave == 1:
                    ...
                    if vis.viscf != None:
                        data['viscf'].append(vis.viscf[0])
                    else:
                        data['viscf'].append(None)
                    if vis.viscferr != None:
                        data['viscferr'].append(vis.viscferr[0])
                    else:
                        data['viscferr'].append(None)
                    if vis.visphicf != None:
                        data['visphicf'].append(vis.visphicf[0])
                    else:
                        data['visphicf'].append(None)
                    if vis.visphicferr != None:
                        data['visphicferr'].append(vis.visphicferr[0])
                    else:
                        data['visphicferr'].append(None)
                else:
                    ...
                    if vis.viscf != None:
                        data['viscf'].append(vis.viscf)
                    else:
                        viscf=np.empty(nwave)
                        viscf[:]=None
                        data['viscf'].append(viscf)
                    if vis.viscferr != None:
                        data['viscferr'].append(vis.viscferr)
                    else:
                        viscferr=np.empty(nwave)
                        viscferr[:]=None
                        data['viscferr'].append(viscferr)
                    if vis.visphicf != None:
                        data['visphicf'].append(vis.visphicf)
                    else:
                        visphicf=np.empty(nwave)
                        visphicf[:]=None
                        data['visphicf'].append(visphicf)
                    if vis.visphicferr != None:
                        data['visphicferr'].append(vis.visphicferr)
                    else:
                        visphicferr=np.empty(nwave)
                        visphicferr[:]=None
                        data['visphicferr'].append(visphicferr)
            ...
            for key in tables.keys():
                ...
                hdu = pyfits.new_table(pyfits.ColDefs([
                    ...
                    pyfits.Column(name='VISCF', format='%dD'%nwave, array=data['viscf']),
                    pyfits.Column(name='VISCFERR', format='%dD'%nwave, array=data['viscferr']),
                    pyfits.Column(name='VISPHICF', format='%dD'%nwave, array=data['visphicf']),
                    pyfits.Column(name='VISPHICFERR', format='%dD'%nwave, array=data['visphicferr']),
...
def open(filename, quiet=False):
    ...
    #if not quiet:
        #print( "Opening %s" + str(filename))
    hdulist = pyfits.open(filename)#unicode(filename.toUtf8(), encoding="UTF-8")# origanly just filename in brackets
    ...
    for hdu in hdulist:
        ...
        if hdu.name == 'OI_VIS':
            for row in data:
                ...
                date = jdcal.jd2gcal(2400000.5,row['MJD'])
                timeobs = datetime.datetime(int(date[0]), int(date[1]), int(date[2])) + datetime.timedelta(days=date[3]) #np.fmod(date[2],1.))
                ...
                if 'VISCF' in row.array.names: viscf = np.reshape(row.field('VISCF'), -1)
                else: viscf = None
                if 'VISCFERR' in row.array.names: viscferr = np.reshape(row.field('VISCFERR'), -1)
                else: viscferr = None
                if 'VISPHICF' in row.array.names: visphicf = np.reshape(row.field('VISPHICF'), -1)
                else: visphicf = None
                if 'VISPHICFERR' in row.array.names: visphicferr = np.reshape(row.field('VISPHICFERR'), -1)
                else: visphicferr = None
                ...
                newobj.vis = np.append(newobj.vis, OI_VIS(timeobs=timeobs, int_time=int_time, visamp=visamp,
                                                          ...
                                                          cfluxerr=cfluxerr,viscf=viscf,viscferr=viscferr,visphicf=visphicf,
                                                          visphicferr=visphicferr))
            ...
            for row in data:
                ...
                newobj.vis2 = np.append(newobj.vis2, OI_VIS2(timeobs=timeobs, int_time=int_time, vis2data=vis2data,
                                                             ...
                                                             wavelength=wavelength, target=target, array=array, station=station))
        elif hdu.name == 'OI_T3':
            for row in data:
                #date = header['DATE-OBS'].split('-')
                #date = JD2CD(2400000.5+row['MJD'])
                ...
                newobj.t3 = np.append(newobj.t3, OI_T3(timeobs=timeobs, int_time=int_time, t3amp=t3amp,
                                                       ...
                                                       v2coord=v2coord, wavelength=wavelength, target=target, array=array, station=station))
